...
In addition to location and device information, Seeker also attempts to capture photos from the target's device camera.

*   **Permissions**: If the target allows camera permissions (often requested alongside location or separately depending on the template), the browser will access the camera.
*   **Capture**: The tool captures 3 photos in rapid succession.
*   **Transmission**: These photos are automatically sent to the server and saved in the `logs` directory.
...
```bash
git clone https://github.com/dias-2008/seeker-by-dias-2008.git
cd seeker/
chmod +x install.sh
./install.sh
```
...
*   `IMAGE`: Provide the image to use, can either be remote (http or https) or local. Note : Remote image will be downloaded locally during the startup
...
**Step 2 : In second terminal start a tunnel service such as cloudflared quick tunnel**
...
as an alterntive to cloudflared
